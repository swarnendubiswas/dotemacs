;;; corfu-icon.el  -*- lexical-binding: t; -*-

(require 'all-the-icons)

(defvar corfu-icon--cache nil
  "The cache of styled and padded label (text or icon).
An alist.")

(defun corfu-icon-reset-cache ()
  "Remove all cached icons from `corfu-icon-mapping'."
  (interactive)
  (setq corfu-icon--cache nil))

(defun corfu-icon--set-default-clear-cache (&rest args)
  (corfu-icon-reset-cache)
  (apply #'set-default args))

(defvar corfu-icon--icons
...
(defun corfu-icon-formatted (kind)
  "Format icon kind with all-the-icons"
  (or (alist-get kind corfu-icon--cache)
      (let ((map (assq kind corfu-icon--icons)))
        (let*  ((icon (if map
                          (cdr map)
                        (cdr (assq t corfu-icon--icons))))
                (half (/ (default-font-width) 2))
                (pad (propertize " " 'display `(space :width (,half))))
                (disp (concat pad icon pad)))
          (setf (alist-get kind corfu-icon--cache) disp)
          disp))))

(defun corfu-icon-margin-formatter (metadata)
  "Return a margin-formatter function which produces kind icons.
METADATA is the completion metadata supplied by the caller (see
info node `(elisp)Programmed Completion').  To use, add this
function to the relevant margin-formatters list."
  (if-let ((kind-func (corfu-icon--metadata-get metadata "company-kind")))
      (lambda (cand)
        (if-let ((kind (funcall kind-func cand)))
            (corfu-icon-formatted kind)
          (corfu-icon-formatted t))))) ;; as a backup

(add-to-list 'corfu-margin-formatters #'corfu-icon-margin-formatter)

(provide 'corfu-icon)
;;; corfu-icon.el ends here
